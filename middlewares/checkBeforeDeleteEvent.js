              ...
              ParticipantModel.countDocuments(query)
                .then((count) => {
                  ...
                  if (count === 0) next();
                    ...
                    res.status(401).send("evenement associé à un participant ");
                })
                .catch((error) => {
                  console.log("Error count participant", error);
                  res.sendStatus(500);
                });
            } else res.status(401).send("evenement associé à un rôle");
          })
          .catch((error) => {
            console.log("Error count role", error);
            res.sendStatus(500);
          });
      } else res.status(401).send("evenement associé à une activité");
    })
    .catch((error) => {
